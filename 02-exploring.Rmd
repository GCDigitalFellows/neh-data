...
In defining the funding analyze, we learn from the [NEH Grants Data Dictionary](https://securegrants.neh.gov/Open/data/NEH_GrantsDictionary.pdf) that there are five different dollar amounts included in the grants dataset:
...
adj <- inflation_adjust(2017) %>%
  data_frame(YearAwarded = seq_along(.) + 1946, Adj = .)
```
